    ...
    <>
      <Box sx={{ mb: 2 }}>
        ...
        >
          ...
          <ToggleButton value="row">Whole Row</ToggleButton>
          <ToggleButton value="col">Whole Column</ToggleButton>
        </ToggleButtonGroup>
      </Box>
...
const AddSectionForm = ({ eventId }) => {
  ...
  const fetchSections = async () => {
    try {
      // ❌ هنا يجب وضع الرابط كسلسلة نصية `` أو ''
  ...
  const handleAddSection = async () => {
    try {
      ...
      // ❌ URL في axios.post يجب أن يكون بين `` لتفسير ${eventId}
